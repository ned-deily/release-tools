...
def buildPythonDocs():
    ...
    if not os.path.exists(htmlDir):
        ...
        runCommand(f"make venv PYTHON={sys.executable}")
...
def buildPythonFramework(python_framework_name, buildDir, configure_options):
    print(
        f"Building {python_framework_name} framework for {UNIVERSALARCHS} architectures in {buildDir}"
    )
    ...
    if main_framework:
        ...
        # work for them.
        runCommand(
            f'ln -s {os.path.join(workDirFrameworks, "Tcl.framework", "Headers", "*")} {workDirUsrLocalInclude}'
        )
        runCommand(
            f'ln -s {os.path.join(workDirFrameworks, "Tk.framework", "Headers", "*")} {workDirUsrLocalInclude}'
        )
        runCommand(
            f'ln -s {os.path.join(workDirFrameworks, "Tcl.framework", "Tcl")} {os.path.join(workDirUsrLocalLib, "libtcl.dylib")}'
        )
        runCommand(
            f'ln -s {os.path.join(workDirFrameworks, "Tk.framework", "Tk")} {os.path.join(workDirUsrLocalLib, "libtk.dylib")}'
        )
        # Remove Tk Wish.app if built
        runCommand(
            f'rm -rf {os.path.join(workDirFrameworks, "Tk.framework", "Resources", "Wish.app")}'
        )
    ...
    runCommand(
        f"{shellQuote(os.path.join(SRCDIR, 'configure'))} "
        f"--enable-framework "
    ...
    if main_framework:
        ...
        runCommand(
            f"make frameworkinstallextras DESTDIR={shellQuote(ROOTDIR)} {runshared_for_make}"
        )
        ...
        if os.path.exists(os.path.join(WORKDIR, "libraries", "Library")):
            ...
            fw_lib_dir = os.path.join(
                ...
                "Library",
                "Frameworks",
                f"{python_framework_name}.framework",
                "Versions",
                getVersion(),
                "lib",
            )
        ...
        if os.path.exists(workDirFrameworks):
            ...
            runCommand(
                f"cp -a {shellQuote(workDirFrameworks)} {shellQuote(fw_pvt_fw_dir)}"
            )

    frmDir = os.path.join(
        ROOTDIR, "Library", "Frameworks", f"{python_framework_name}.framework"
    )
    ...
    if not main_framework:
        main_framework_bin = os.path.join(
            ROOTDIR,
            "Library",
            "Frameworks",
            "Python.framework",
...
def main():
    ...
    if os.path.exists(THIRD_PARTY_LIBRARIES_CACHE):
        print(
            f" -- WARNING: using pre-built third-party libraries from {THIRD_PARTY_LIBRARIES_CACHE}\n"
        )
        runCommand(
            f'cp -a {THIRD_PARTY_LIBRARIES_CACHE} {os.path.join(WORKDIR, "libraries")}'
        )
    else:
    ...
    runCommand(
        f'cp -a {os.path.join(WORKDIR, "libraries")} {os.path.join(WORKDIR, "libraries-saved")}'
    )
